    ...
    int CProcessRawEpigraphData::ProcessFile(wchar_t* rawDataFilePath)
        {
            ...
            // макс c запасом чатоста 500гц
            DWORD modulatorPulseTime[20000] = {NULL};
            ...
            while(inClientFile>>eventID &&
                ...
                !inClientFile.eof())
            {
                if(time[1]<time[0]) continue;// отсортируем отруем запаздывающие записи от слишком длинных импульсов
                switch(eventID)
                {
...
//============Distrebution with const step for periods=============================
                ...
                HalfWindTimeInChopPrdDestrib = 1000.*m_AdcRate/(rotFrequency - m_HalfWindInChopFreqDestrib) - avrPrd;
...
//                                        chopPeriodDistr);
            ...
            DWORD collectionTimeInSec[200] = {NULL},inThisFileCollTimeInSec( measTimeInFileInADCTicks/m_AdcRate/1000 );
...
//                }
    ...
    int CProcessRawEpigraphData::ProcessDirectory(wchar_t * dirPath)
    {
        ...
        if(dir)
        {
           // send signal that file has processed
           emit ProcessedFile();
           struct dirent *ent;
           int error(0);
    ...
    void CProcessRawEpigraphData::CulculateNumOfRawFiles(wchar_t * dirPath)
    {
        char dirPathBuffer[PATH_MAX] = {NULL};
        wcstombs(dirPathBuffer,dirPath,sizeof(dirPathBuffer));
        WORD i(0);
    ...
    void CProcessRawEpigraphData::Stop()
    {
        m_StopFlag = true;
    }
